...
def main():
    ...
    username = f"uxflow{int(time.time())}"
    password = "Pass0630!"
    ...
    with sync_playwright() as p:
        ...
        page.evaluate(
            "(tk) => { localStorage.setItem('gbt_auth_token', tk); }",
            token,
        )
        ...
        page.fill("#chat-input", "浏览器级复检：登录后聊天是否正常")
        ...
        result = {
            "active_tab": page.locator(".n-item.active").first.get_attribute("data-tab"),
            "auth_state": txt(page, "#auth-state"),
            "auth_remaining": txt(page, "#auth-remaining"),
            "chat_auth_tip": txt(page, "#chat-auth-tip"),
            "chat_last": txt(page, "#chat-messages .chat-msg:last-child .msg-text"),
