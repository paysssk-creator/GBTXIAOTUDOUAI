...
export default function MarketData() {
  const { data: indices, loading } = useMarketIndices();
  const gainers = indices.filter(i => Number(i.change_pct) > 0).length;
  const losers = indices.filter(i => Number(i.change_pct) < 0).length;
  const flat = indices.filter(i => Number(i.change_pct) === 0).length;
                ...
                return (
                  <div key={idx.code} className="grid grid-cols-7 gap-2 px-5 py-4 hover:bg-surface-overlay transition-colors">
                    ...
                    <div className={cn("font-mono text-sm self-center", up ? "text-gain" : dn ? "text-loss" : "text-muted-foreground")}>
                      {up ? "+" : ""}{Number(idx.change).toFixed(2)}
                    </div>
                    ...
                    <div className="self-center space-y-0.5">
                      <div className="font-mono text-xs text-gain">{Number(idx.high).toFixed(2)}</div>
                      <div className="font-mono text-xs text-loss">{Number(idx.low).toFixed(2)}</div>
                    </div>
                  </div>
                );
              })}
